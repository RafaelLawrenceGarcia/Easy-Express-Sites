...
//  User lands here after clicking the PlayFab recovery email link.
//  URL format: https://yoursite.com/?token=XXXX&lang=en#/reset-confirm
//  Reads token from window.location.search, calls Client/ResetPassword.
// ============================================================
...
function PwStrength({ password }) {
  ...
  const levels = [
    { label: "WEAK", color: A2 }, { label: "WEAK", color: A2 },
    { label: "FAIR", color: WN }, { label: "GOOD", color: A },
    { label: "STRONG", color: OK }, { label: "STRONG", color: OK }
  ];
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
        {[1,2,3,4].map(i => (
          <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= Math.min(score,4) ? s.color : BD }} />
        ))}
      </div>
...
export default function ResetConfirmPage() {
  ...
  useEffect(() => {
    // Token is in query string (?token=XXX) before the hash
  ...
  const pwToggle = (active) => ({
    position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)",
    background: "none", border: "none", color: active ? A : TD,
    cursor: "pointer", fontFamily: F1, fontSize: 11, fontWeight: 700, padding: 0
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/Client/ResetPassword`, {
        ...
        body: JSON.stringify({ Token: token, Password: newPassword, TitleId: TITLE_ID })
      });

      // PlayFab returns empty body on success — don't parse JSON
      ...
      try {
        ...
        throw new Error(json.errorMessage || json.error || `Error ${json.code}`);
      } catch (e) {
        ...
        <div style={{ background: CARD, border: `1px solid ${BD}`, borderRadius: 20, padding: "40px 36px", position: "relative", overflow: "hidden", boxShadow: `0 24px 80px ${BG}cc` }}>
          ...
          {noToken && !done && (
            <div style={{ textAlign: "center" }}>
              ...
              <p style={{ color: TD, fontSize: 14, fontFamily: F1, lineHeight: 1.7, marginBottom: 28 }}>
                No reset token was found in the URL. This link may have expired or already been used.
              </p>
            ...
            <div style={{ animation: "fadeSlideUp 0.4s ease-out" }}>
              ...
              <button onClick={handleSubmit}
                ...
                style={{
                  ...
                  background: newPassword.length >= 8 && newPassword === confirmPw
                    ? `linear-gradient(135deg, ${OK}, ${A})` : BD,
                  color: newPassword.length >= 8 && newPassword === confirmPw ? BG : TD,
                  fontFamily: F1, fontWeight: 800, fontSize: 15, letterSpacing: 1.5,
                  cursor: loading || newPassword.length < 8 || newPassword !== confirmPw ? "not-allowed" : "pointer",
          ...
          {done && (
            <div style={{ textAlign: "center", animation: "fadeSlideUp 0.4s ease-out" }}>
              ...
              <p style={{ color: TD, fontSize: 14, fontFamily: F1, lineHeight: 1.7, marginBottom: 32 }}>
